...
public class BaseManageAction {
	...
	public void logout_success(HttpServletRequest request, HttpServletResponse response) throws IOException {
		 ...
		 * "wemall_view_type")); //跳转到微信端 if ((wemall_view_type != null) &&
		 * (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))) {
	...
	@RequestMapping({ "/pay_error.htm" })
	public ModelAndView pay_error(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("wap/pay_error.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		mv.addObject("url", CommUtil.getURL(request) + "/wap/index.htm");
	...
	@RequestMapping({ "/pay_success.htm" })
	public ModelAndView pay_success(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("wap/pay_error.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		mv.addObject("url", CommUtil.getURL(request) + "/wap/index.htm");
