...
public class UserManageAction {
	...
	public ModelAndView user_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		if (condition != null) {
			if (condition.equals("mobile")) {
				uqo.addQuery("obj.mobile", new SysMap("mobile", value), "=");
			}
	...
	public ModelAndView user_save(HttpServletRequest request, HttpServletResponse response, String id, String role_ids,
			String list_url, String add_url, String password, String xihaoBalance) {
		WebForm wf = new WebForm();
		User user = null;
		if (id.equals("")) {
		...
		if (!StringUtils.isBlank(xihaoBalance)) {
			BigDecimal xihaoBalance_float = new BigDecimal(0);
			try {
				 xihaoBalance_float = BigDecimal.valueOf(Double.valueOf(xihaoBalance).doubleValue());
			} catch (Exception e) {
				xihaoBalance_float = new BigDecimal(0);
			}
			user.setXihaoBalance(xihaoBalance_float);
		}
		if (id.equals("")) {// 新增会员
