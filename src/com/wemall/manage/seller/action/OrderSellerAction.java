...
public class OrderSellerAction {
	...
	public ModelAndView order(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String order_status, String order_id, String beginTime, String endTime, String buyer_userName, String type, String type_data) {
		...
		ofqo.addQuery("obj.store.user.id", new SysMap("user_id", userId), "=");
		...
		if (!CommUtil.null2String(buyer_userName).equals("")) {
		 ...
		 if (!CommUtil.null2String(type_data).equals("")) {
	            ...
	            // 收件人姓名
	            if (type.equals("addrMobile")) {
	            	ofqo.addQuery("obj.addr.mobile", new SysMap("mobile", "%" + type_data + "%"), "like");
	            }
	        }
	...
	public String order_refund_save(HttpServletRequest request, HttpServletResponse response, String id,
		...
		if (obj.getStore().getId().equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
			...
			if (type.equals(refund_type)) {
				...
				if (null == seller.getAvailableBalance()) {
					seller.setAvailableBalance(new BigDecimal(0));
				}
				...
				if (null == buyer.getAvailableBalance()) {
					buyer.setAvailableBalance(new BigDecimal(0));
				}
			...
			if (type.equals(refund_type)) {
				...
				this.userService.update(seller);
				User buyer = obj.getUser();
				if (null == buyer.getXihaoBalance()) {
					buyer.setXihaoBalance(new BigDecimal(0));
				}
				buyer.setXihaoBalance(BigDecimal.valueOf(
						CommUtil.add(buyer.getXihaoBalance(), BigDecimal.valueOf(CommUtil.null2Double(refund)))));
				this.userService.update(buyer);
			}

			// ($!obj.order_status>=20 && $!{obj.order_status}<50 && $!remain>0)
			// TODO 设置订单的状态
			// $remain=$!{obj.totalPrice}-$!CommUtil.null2Double($!{obj.refund}))
			double totalPrice = 0;
			try {
				totalPrice = obj.getTotalPrice().doubleValue();
			} catch (Exception e) {
				totalPrice = 0;
			}
			...
			double refund1 = 0;
			try {
				refund1 = obj.getRefund().doubleValue();
			} catch (Exception e) {
				refund1 = 0;
			}
			if (obj.getOrder_status() < 20 || obj.getOrder_status() > 50 || totalPrice <= refund1) {
				obj.setOrder_status(0);
				orderFormService.update(obj);
			}
