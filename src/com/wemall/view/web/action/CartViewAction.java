...
import com.wemall.apis.jd.response.CheckSkuResponse.Result;
...
import com.wemall.core.tools.StringUtils;
...
public class CartViewAction {
    ...
    public void remove_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String store_id,
                                  String idlist) {
        if (org.apache.commons.lang3.StringUtils.isBlank(id) && org.apache.commons.lang3.StringUtils.isBlank(idlist)) {
            return;
        }
        String[] ids = new String[] {};
        if (!org.apache.commons.lang3.StringUtils.isBlank(idlist)) {
            ids = idlist.split(",");
        }
        ...
        for (String key : list) {
            ...
            map = removeGoodsCartById(request, key, store_id);
        }
    ...
    private Map<String, Object> removeGoodsCartById(HttpServletRequest request, String id, String store_id) {
    ...
    public ModelAndView goods_cart3(HttpServletRequest request, HttpServletResponse response, String cart_session,
        ...
        List<StoreCart> cart = cart_calc(request);
        // 判断京东商品是否可售
        CheckSkuResponse checkSkuResponseForJd = getCheckSkuResponseForJd(cart);
        if (checkSkuResponseForJd != null && checkSkuResponseForJd.isSuccess()
                && !checkSkuResponseForJd.getResult().isEmpty()) {
            for (Result r : checkSkuResponseForJd.getResult()) {
                // 下架
                if (r.getIsCanVAT() == 0 || r.getSaleState() == 0) {
                    mv = new JModelAndView("error.html", this.configService.getSysConfig(),
                            this.userConfigService.getUserConfig(), 1, request, response);
                    ...
                    // 将下架的商品标示为已下架
                    Goods goods = goodsService.getObjByProperty("goodsSkuId", r.getSkuId() + "");
                    if (null != goods) {
                        goods.setGoods_status(0);
                        this.goodsService.update(goods);// 更新商品资料

                        // 更新全文检索
                        String goods_lucene_path = (new StringBuilder(
                                String.valueOf(System.getProperty("wemall.root")))).append(File.separator)
                                .append("lucene").append(File.separator).append("goods").toString();
                    ...
                    mv.addObject("op_title", "商品:【" + r.getName() + "】已下架！");
                    mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
                    return mv;
                }
            }
        ...
        CheckStockResponse checkStockResponse = getStockByIdResponse(cart);
        if (checkStockResponse != null && checkStockResponse.isSuccess() && !checkStockResponse.getResult().isEmpty()) {
            for (com.wemall.apis.jd.response.CheckStockResponse.Result r : checkStockResponse.getResult()) {
                if (r.getState() == 33 || r.getState() == 39 || r.getState() == 40) {
                    continue;
                }
                // 下架
                mv = new JModelAndView("error.html", this.configService.getSysConfig(),
                ...
                // 将下架的商品标示为已下架
                Goods goods = goodsService.getObjByProperty("goodsSkuId", r.getSku() + "");

                mv.addObject("op_title", "商品:【" + goods.getGoods_name() + "】无货！");
                mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
                return mv;
            }
        }
    ...
    private CheckSkuResponse getCheckSkuResponseForJd(List<StoreCart> cart) {
        List<String> skuIds = new ArrayList<String>();
        for (StoreCart sc : cart) {
            if (sc.getStore().getId().longValue() == 30l) {
                for (GoodsCart gc : sc.getGcs()) {
                    if (gc.getGoods() != null
                            && !org.apache.commons.lang3.StringUtils.isEmpty(gc.getGoods().getGoodsSkuId())) {
                        skuIds.add(gc.getGoods().getGoodsSkuId());
                    }
                }
            }
        }
        return jdGoodsService.getCheckSkuResponse(skuIds);
    }

    private CheckStockResponse getStockByIdResponse(List<StoreCart> cart) {
        List<String> skuIds = new ArrayList<String>();
        for (StoreCart sc : cart) {
            if (sc.getStore().getId().longValue() == 30l) {
                for (GoodsCart gc : sc.getGcs()) {
        ...
        return jdGoodsService.getStockByIdResponse(skuIds);
    }
    ...
    public ModelAndView order_pay(HttpServletRequest request, HttpServletResponse response, String payType,
        ...
        if (of.getOrder_status() == 10) {
            if (CommUtil.null2String(payType).equals("")) {
                ...
                if (StringUtils.endsWithIgnoreCase("xihaocard", payType)) {
                    params.clear();
                    params.put("mark", payType);
                ...
                if (payType.equals("balance")) {
                    mv = new JModelAndView("balance_pay.html", this.configService.getSysConfig(),
                            this.userConfigService.getUserConfig(), 1, request, response);
                } else if (payType.equals("xihaocard")) {
                    try {
                        ModelAndView xihao_pay_balance = xihao_pay_balance(request, response, payType, order_id);
                        return xihao_pay_balance;
                    } catch (Exception e) {
                        e.printStackTrace();
                        mv = new JModelAndView("wap/error.html", this.configService.getSysConfig(),
                                this.userConfigService.getUserConfig(), 1, request, response);
        ...
        if (of != null && of.getOrder_status() == 10) {
            ...
            if (payments.size() > 0) {
                ...
                if (payType.equals("weixin_wap")) {
                    ...
                    // TODO 锡好卡支付
                    try {
                        ModelAndView mv = xihao_pay_balance(request, response, payType, order_id);
                        Map<String, Object> model = mv.getModel();
                        String op_title = (String) model.get("op_title");
                        if (op_title.equalsIgnoreCase("锡好卡余额不足，支付失败！")) {
                            return "redirect:" + CommUtil.getURL(request) + "/pay_error.htm";
                        }
                        // return mv;
                    } catch (Exception e) {
                        e.printStackTrace();
                    ...
                    // 正常跳转
                    return "redirect:" + CommUtil.getURL(request) + "/pay_success.htm";
                } else {
                    // 支付方式错误
    ...
    public ModelAndView xihao_pay_balance(HttpServletRequest request, HttpServletResponse response, String payType,
        ...
        if (CommUtil.null2Double(user.getXihaoBalance()) < CommUtil.null2Double(of.getTotalPrice())) {
            mv = new JModelAndView("error.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 1, request, response);
            mv.addObject("op_title", "锡好卡余额不足，支付失败！");
            mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
            return mv;
        }
        // 锡好卡余额可以支付订单
        ...
        Map params = new HashMap();
        params.put("mark", "xihaocard");
        List payments = this.paymentService.query("select obj from Payment obj where obj.mark=:mark", params, -1, -1);
        if (payments.size() > 0) {
            of.setPayment((Payment) payments.get(0));
            of.setPayTime(new Date());
        }
        ...
        if (ret) {
            user.setXihaoBalance(BigDecimal.valueOf(CommUtil.subtract(user.getXihaoBalance(), of.getTotalPrice())));
            ...
            log.setPd_log_info("订单" + of.getOrder_id() + "购物减少可用锡好卡余额");
            log.setPd_type("锡好卡余额");
            ...
            for (GoodsCart gc : of.getGcs()) {
                ...
                Map temp;
                if (null == goods.getInventory_type() || goods.getInventory_type().equals("all")) {
        ...
        OrderFormLog ofl = new OrderFormLog();
        ofl.setAddTime(new Date());
        ofl.setLog_info("锡好卡余额支付");
        ofl.setLog_user(SecurityUserHolder.getCurrentUser());
        ofl.setOf(of);
        this.orderFormLogService.save(ofl);
        mv.addObject("op_title", "锡好卡余额支付成功！");
        mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
        return mv;
    }
        ...
        if (CommUtil.null2Double(user.getAvailableBalance()) > CommUtil.null2Double(of.getTotalPrice())) {
            ...
            if (ret) {
                ...
                for (GoodsCart gc : of.getGcs()) {
                    ...
                    goods.setGoods_salenum(goods.getGoods_salenum() + gc.getCount());
                    Map temp;
                    if (org.apache.commons.lang3.StringUtils.isBlank(goods.getInventory_type())
                            || goods.getInventory_type().equals("all")) {
                        goods.setGoods_inventory(goods.getGoods_inventory() - gc.getCount());
                    } else {
                        List list = (List) Json.fromJson(ArrayList.class, goods.getGoods_inventory_detail());
                        for (Iterator localIterator4 = list.iterator(); localIterator4.hasNext();) {
