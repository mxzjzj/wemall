	...
	public ModelAndView goods(HttpServletRequest request, HttpServletResponse response, String id) {
		...
		if (obj.getGoods_status() == 0) {// 商品上架状态
			...
			if (obj.getGoods_store().getStore_status() == 2) {// 店铺状态正常
				...
				// 获取店铺商品数量
				long storeId = obj.getGoods_store().getId();
				int storeGoodsCount = jdbcTemplate.queryForInt("SELECT count(*) FROM wemall_goods WHERE goods_store_id='"+storeId+"' AND deleteStatus='0' AND goods_status='0'");
				mv.addObject("store_goods_count", storeGoodsCount);
...
//					current_city = ipAddr.IpStringToAddress(current_ip);
...
//				}
	...
	public ModelAndView ztc_goods_list(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType, String goods_view) {
		ModelAndView mv = new JModelAndView("ztc_goods_list.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		GoodsQueryObject gqo = new GoodsQueryObject(currentPage, mv, orderBy, orderType);
		gqo.addQuery("obj.goods_status", new SysMap("goods_status", Integer.valueOf(0)), "=");
	...
	public void trans_fee(HttpServletRequest request, HttpServletResponse response, String city_name, String goods_id) {
		...
		if (goods.getTransport() != null) {
			if (goods.getTransport().getTrans_mail_info() != null) {
				mail_fee = this.transportTools.cal_goods_trans_fee(CommUtil.null2String(goods.getTransport().getId()), "mail", CommUtil.null2String(goods.getGoods_weight()), CommUtil.null2String(goods.getGoods_volume()), city_name);
			}
			if (goods.getTransport().getTrans_express_info() != null) {
				express_fee = this.transportTools.cal_goods_trans_fee(CommUtil.null2String(goods.getTransport().getId()), "express", CommUtil.null2String(goods.getGoods_weight()), CommUtil.null2String(goods.getGoods_volume()), city_name);
			}
			if (goods.getTransport().getTrans_ems_info() != null) {
				ems_fee = this.transportTools.cal_goods_trans_fee(CommUtil.null2String(goods.getTransport().getId()), "ems", CommUtil.null2String(goods.getGoods_weight()), CommUtil.null2String(goods.getGoods_volume()), city_name);
			}
		}
