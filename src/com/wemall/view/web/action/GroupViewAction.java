...
public class GroupViewAction {
	...
	public ModelAndView group_view(HttpServletRequest request, HttpServletResponse response, String id) {
		...
		if (view) {
			...
			List<GoodsCart> gc_list = this.goodsCartService.query(
					"select obj from GoodsCart obj where :gg member of obj.goods.group_goods_list", params, -1, -1);
	...
	@RequestMapping({ "/wap/group_view.htm" })
	public ModelAndView wap_group_view(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("wap/group_view.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
