...
public class PayViewAction {
    ...
    //zhangjun
    @RequestMapping({"/gdpay.htm"})
    public void gdpay(HttpServletRequest request, HttpServletResponse response, String id, String type, String payment_id) throws IOException {
        OrderForm of = null;
        Predeposit obj = null;
        GoldRecord gold = null;
        IntegralGoodsOrder ig_order = null;
        of = this.orderFormService.getObjById(CommUtil.null2Long(id));
        String order_price = CommUtil.null2String(of.getTotalPrice());
        Date addtime = of.getAddTime();
        String transDateTime = TenpayUtil.date2String(addtime, "yyyyMMddHHmmss");
        String customerName = of.getAddr().getTrueName();

        double total_fee = CommUtil.null2Double(order_price) * 100.0D;
        int fee = (int) total_fee;
//        String product_name = of.getOrder_id();
        String remarkexplain = of.getMsg();
        String attach = attach = type + "," + of.getId().toString();
        String desc = "商品：" + id;

        RequestHandler reqHandler = new RequestHandler(request, response);
        reqHandler.init();

        reqHandler.setGateUrl("https://111.205.51.141/per/preEpayLogin.do?_locale=zh_CN");

        reqHandler.setParameter("transId", "IPER");
        reqHandler.setParameter("merchantId", "370310000004");//399110000003
        reqHandler.setParameter("orderId", id);
        reqHandler.setParameter("transAmt", String.valueOf(fee));
        reqHandler.setParameter("transDateTime", transDateTime);
        reqHandler.setParameter("currencyType", "01");
        reqHandler.setParameter("customerName", customerName);
        reqHandler.setParameter("merSecName", "锡好");
        reqHandler.setParameter("productInfo", desc);
        reqHandler.setParameter("payIp", request.getRemoteAddr());
        reqHandler.setParameter("merURL", CommUtil.getURL(request) + "/gdpay_return.htm");
        reqHandler.setParameter("merURL1", CommUtil.getURL(request) + "/gdpay_return.htm");
        reqHandler.setParameter("msgExt", "锡好");

        String requestUrl = reqHandler.getRequestURL_GD();
    ...
    @RequestMapping({"/gdpay_return.htm"})
    public ModelAndView gdpay_return(HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        ModelAndView mv = new JModelAndView("order_finish.html",
                this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        String trade_no = request.getParameter("transSeqNo");
        String orderId = request.getParameter("orderId");
        String total_fee = request.getParameter("transAmt");

        String trade_status = request.getParameter("ResponseCode");
        OrderForm order = null;
        Predeposit obj = null;
        GoldRecord gold = null;
        IntegralGoodsOrder ig_order = null;
        order = this.orderFormService.getObjById(CommUtil.null2Long(orderId));
